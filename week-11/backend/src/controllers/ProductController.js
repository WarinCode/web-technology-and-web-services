...
export default class ProductController {
  ...
  //* GET: http://localhost:3000/products/three
  async getThreeProducts(req, res) {
    try {
      const query = `
      SELECT p.*,
        (
      ...
      FROM products p ORDER BY "pdId" OFFSET 0 LIMIT 3
          `;
      const result = await database.query(query);
      res.status(200).json(result.rows);
    } catch (err) {
  ...
  async getSearchProduct({ params: { id } }, res) {
    try {
